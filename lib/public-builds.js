...
export function computeBuildSummary(token, itemData, skillsData) {
    ...
    for (const key of Object.values(data.items)) {
        if (!key || key === 'None') continue;
        const item = itemData[key];
        if (item) {
            itemNames.push(item.name || key);
    ...
    // rarity level. The tree is the first main tree holding one of them.
    let czTree = null;
    const czNames = data.czAbilities.map((c) => c.name);
    if (czNames.length > 0) {
    ...
    for (const c of data.czAbilities) {
        skillEntries.push({ n: skillAbbr(c.name, c.name), f: c.name, r: Number(c.rarity) || 0, g: 'c' });
    }
    ...
    return {
        ...
        items_json: JSON.stringify([...itemNames, ...charmNames]),
    };
}
