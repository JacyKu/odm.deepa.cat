...
export function saveBuild({ state, userId, name, notes }) {
    ...
    db.prepare(
        'INSERT INTO builds (id, token, user_id, state, name, notes, creator_token, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, datetime(\'now\'), datetime(\'now\'))'
    ).run(
        id,
...
export function updateBuildState(id, userId, creatorToken, { state, name, notes } = {}) {
    if (!id) return false;
    const canonical = canonicalState(state);
