...
export function getBuild(id) {
    ...
    const row = db
        .prepare(
            'SELECT id, token, user_id, state, name, notes, created_at, updated_at, is_public, anonymous, author_name, author_avatar, class_name, spec, region, power, has_charms, masterwork_count, cz_tree, skills_json, items_json FROM builds WHERE id = ?'
        )
        .get(id);
    if (!row) return null;
