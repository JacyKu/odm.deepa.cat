...
import { getLinkPreviewData } from '../../../_src/utils/buildPreview';
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: 2 }}>
      {Array.from({ length: max }, (_, i) => (
        <svg key={i} width={size} height={size} viewBox="0 0 576 512">
          <path
            d={STAR_PATH}
            fill={i < count ? STAR : 'rgba(255,255,255,0.15)'}
          />
        </svg>
      ))}
    </div>
          ...
          {pair.map(({ label, name, img, ex, tier, masterwork }) => (
            <div key={label} style={{ display: 'flex', width: CELL_W, marginRight: 14 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', marginLeft: 12 }}>
                <div style={{ fontSize: 12, letterSpacing: 1.5, color: DIM, fontWeight: 700 }}>{label.toUpperCase()}</div>
                <div style={{ display: 'flex', alignItems: 'center', fontSize: 17, marginTop: 1 }}>
                  {ex && <div style={{ color: ACCENT, fontWeight: 800, letterSpacing: 1, marginRight: 6 }}>EX</div>}
                  <div style={{ color: name ? TEXT : DIM, fontWeight: name ? 600 : 400 }}>{name || 'None'}</div>
                </div>
  ...
  return (
    <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
      {skills.map((s) => {
        ...
        return (
          <div key={s.id} style={{ display: 'flex', alignItems: 'center', gap: 5, border: `1px solid ${BORDER}`, background: PANEL, padding: '3px 9px', fontSize: 14, color: chipColor }}>
            <span style={{ fontWeight: 600 }}>{s.name || s.shortName || s.id}</span>
            <span style={{ fontWeight: 800 }}>{s.points}{enhanced ? '*' : ''}</span>
          </div>
        );
      })}
    </div>
...
export async function GET(request) {
  ...
  if (!data) {
    ...
    return new ImageResponse(
      (
        <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: '#0e0e14', color: TEXT, ...fontStyle }}>
          {favicon && (
            <img src={favicon} width={96} height={96} style={{ imageRendering: 'pixelated' }} />
          )}
          <div style={{ fontSize: 28, letterSpacing: 8, color: ACCENT, fontWeight: 700, marginTop: 28 }}>Ode to Misery</div>
  ...
  const title = data.name || 'Monumenta Builder';
  ...
  const itemLines = data
    ? await Promise.all(
        Object.entries(SLOT_LABELS).map(async ([slot, label]) => {
          ...
          if (key === 'None') return { label, name: null, key: null, img: null, ex: false, tier: null, masterwork: 0 };
  ...
  return new ImageResponse(
    (
      <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', background: '#0e0e14', color: TEXT, padding: '40px 56px', boxSizing: 'border-box', ...fontStyle }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
          <div style={{ fontSize: 16, letterSpacing: 4, color: DIM, fontWeight: 700 }}>Ode to Misery</div>
        </div>
        ...
        <div style={{ display: 'flex', gap: 28, marginTop: 'auto', paddingTop: 20 }}>
          <div style={{ flex: 1, border: `2px solid ${BORDER}`, background: PANEL, padding: 20, display: 'flex', flexDirection: 'column', gap: 12 }}>
            <div style={{ fontSize: 15, letterSpacing: 2, color: DIM, fontWeight: 700 }}>EQUIPMENT</div>
            <EquipmentGrid itemLines={itemLines} />
          </div>

          {charmNames.length > 0 && (
            <div style={{ width: 330, border: `2px solid ${BORDER}`, background: PANEL, padding: 20, display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div style={{ fontSize: 15, letterSpacing: 2, color: DIM, fontWeight: 700, marginBottom: 2 }}>CHARMS</div>
              {charmNames.map((c, i) => (
                <div key={i} style={{ display: 'flex', flexDirection: 'column' }}>
                  <div style={{ fontSize: 16, color: TEXT }}>{c.name}</div>
                  <div style={{ display: 'flex', alignItems: 'center', marginTop: 3 }}>
                    {c.power != null && <Stars filled={c.power} max={c.power} size={10} />}
                    {c.power != null && <div style={{ color: DIM, fontSize: 12, marginLeft: 8 }}>{String(c.power)}</div>}
                  </div>
                </div>
              ))}
            </div>
